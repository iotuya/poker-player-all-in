...
public class Player {
    ...
    public static int betRequest(JsonElement request) {
        ...
        if (round == 0) {
            ...
            if (!cards.isEmpty()) {
                ...
                if (betIndex > 0) {
                    raise = continuedBettingStrategy(request, buyIn, hand, cards, minimumRaise, round, rank, value);
                } else {
    ...
    private static int continuedBettingStrategy(JsonElement request, int buyIn, Hand hand, List<Card> cards, int minimumRaise, int round, int rank, int value) {
